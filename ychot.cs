...
namespace triumph
{
    public partial class ychot : Form
    {
        public ychot()
        {
        ...
        {
            SqlCommand com = new SqlCommand("select idos as'Номер отпуска',nameorg as'Наименование организации',fiodir as'Фио директора',fiocot as'Фио сотрудника',position as'Должность',daten as'Дата начала',datek as'Дата окончания',kolday as'Количество дней',nameych as'Наименование учебного заведения',spec as'Специальность/курс' from ycheb", conn);
        ...
        private void Button2_Click(object sender, EventArgs e)
        {
            try
            {
                string insert = "insert into ycheb values (@nameorg,@fiodir,@fiocot,@position,@daten,@datek,@kolday,@nameych,@spec)";
                com.Connection = conn;
                conn.Open();
                using (SqlCommand command = new SqlCommand(insert, conn))
                {
                    ...
                    command.Parameters.AddWithValue("@nameych", textBox9.Text);
                    command.Parameters.AddWithValue("@spec", textBox10.Text);
        ...
        private void Button3_Click(object sender, EventArgs e)
        {
            try
            {
                string update = "update ycheb set nameorg=@nameorg,fiodir=@fiodir,fiocot=@fiocot,position=@position,daten=@daten,datek=@datek,kolday=@kolday,nameych=@nameych,spec=@spec where idos=@idos";
                com.Connection = conn;
                conn.Open();
                using (SqlCommand com = new SqlCommand(update, conn))
                {
                    ...
                    com.Parameters.AddWithValue("@nameych", dataGridView1.Rows[index].Cells[8].Value);
                    com.Parameters.AddWithValue("@spec", dataGridView1.Rows[index].Cells[9].Value);
                    com.ExecuteNonQuery();
        ...
        private void Button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string del = ("delete from ycheb where idos='" + value + "'");
                com.Connection = conn;
                com.CommandText = (del);
                conn.Open();
                com.ExecuteNonQuery();
        ...
        private readonly string examp = @"D:\Работа\Крылова А.А. дипломная работа\ШаблонОтпускаУчебного.docx";
        ...
        private void Button5_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                WordDocument.SaveAs(@"D:\Работа\Крылова А.А. дипломная работа\ДоговорОтпускаУчебного.docx");
