...
namespace triumph
{
    public partial class zayavlenienawork : Form
    {
        public zayavlenienawork()
        {
        ...
        {
            SqlCommand com = new SqlCommand("select idos as'Номер отпуска',nameorg as'Наименование организации',fiodir as'Фио директора',fiocot as'Фио сотрудника',nameotd as'Наименование отдела',position as'Должность',datepr as'Дата приема' from priem", conn);
        ...
        private void Button2_Click(object sender, EventArgs e)
        {
            try
            {
                string insert = "insert into priem values (@nameorg,@fiodir,@fiocot,@nameotd,@position,@datepr)";
                com.Connection = conn;
                conn.Open();
                using (SqlCommand command = new SqlCommand(insert, conn))
                {
                    ...
                    command.Parameters.AddWithValue("@nameotd", textBox5.Text);
                    command.Parameters.AddWithValue("@position", textBox6.Text);
                    command.Parameters.AddWithValue("@datepr", textBox7.Text);
        ...
        private void Button3_Click(object sender, EventArgs e)
        {
            try
            {
                string update = "update priem set nameorg=@nameorg,fiodir=@fiodir,fiocot=@fiocot,nameotd=@nameotd,position=@position,datepr=@datepr where idos=@idos";
                com.Connection = conn;
                conn.Open();
                using (SqlCommand com = new SqlCommand(update, conn))
                {
                    ...
                    com.Parameters.AddWithValue("@nameotd", dataGridView1.Rows[index].Cells[4].Value);
                    com.Parameters.AddWithValue("@position", dataGridView1.Rows[index].Cells[5].Value);
                    com.Parameters.AddWithValue("@datepr", dataGridView1.Rows[index].Cells[6].Value);
        ...
        private void Button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string del = ("delete from priem where idos='" + value + "'");
                com.Connection = conn;
                com.CommandText = (del);
                conn.Open();
                com.ExecuteNonQuery();
        ...
        private readonly string examp = @"D:\Работа\Крылова А.А. дипломная работа\ШаблонПриемаНаРаботу2.docx";
        ...
        private void Button5_Click(object sender, EventArgs e)
        {
            ...
            var cl8 = Convert.ToString(this.dataGridView1.CurrentRow.Cells[3].Value);
            var cl9 = Convert.ToString(this.dataGridView1.CurrentRow.Cells[1].Value);
            ...
            try
            {
                ...
                WordDocument.SaveAs(@"D:\Работа\Крылова А.А. дипломная работа\ДоговорПриемаНаРаботу.docx");
